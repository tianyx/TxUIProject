// FileDriver_AVI.cpp: implementation of the CFileDriver_AVI class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
//#include "FCVSServer.h"
#include "FileDriver_AVI.h"
...
int CFileDriver_AVI::m_nNum = 0;
REGISTER_CLASS(CFileDriver_AVI)
CFileDriver_AVI::CFileDriver_AVI() : CFileDriver(&m_DriverMgr_AVI)
{
	m_strTskID = "";
	m_strCheckKind = "";
	m_strFileType = "";
...
CFileDriver_AVI::~CFileDriver_AVI()
{
	OutputDebugString("into CFileDriver_WMV::~CFileDriver_AVI()\n");
}


void CFileDriver_AVI::ShowMsg()
{
	AfxMessageBox(m_strMsg);
}

BOOL CFileDriver_AVI::InitTask(CString strXML)
{
	if (strXML.IsEmpty())
	{
		return FALSE;
    ...
    CString strDriverPara   = GetFCVSServerConfig().GetXmlByClassName("CFileDriver_AVI");
	...
	m_DriverMgr_AVI.SetCallBack((JS_CCallBack*)this);
	m_DriverMgr_AVI.SetBuffer((void*)&m_SampleList);
	m_DriverMgr_AVI.SetParent(m_hWnd);
	///add end
...
BOOL CFileDriver_AVI::RunTask()
{
	BOOL bRet = m_DriverMgr_AVI.Run();
	if (bRet)
	{
...
BOOL CFileDriver_AVI::PauseTask()
{
	BOOL bRet = m_DriverMgr_AVI.Pause();
	if (bRet)
	{
		// wang.zh  commented . 暂停任务还要设置返回类型？
...
BOOL CFileDriver_AVI::CancelTask()
{
	return m_DriverMgr_AVI.Cancel();
}

CString CFileDriver_AVI::GetTskID()
{
	return m_strTskID;
}

void CFileDriver_AVI::SetParentWnd(HWND hwnd)
{
	m_hParentWnd = hwnd;
}

HRESULT CFileDriver_AVI::GetFileInfo(JS_StreamInfo p)
{
	m_StreamInfo = p;
...
LONGLONG CFileDriver_AVI::GetFileDuration()
{
	return m_StreamInfo.llFileDuration;
}

BOOL CFileDriver_AVI::GetFileInfoByArr(CStringArray& strFileInfoFiledArr,CStringArray& strFileInfoValueArr,BOOL bXML /* = TRUE */)
{
	CString strTemp;
	if (bXML)
	{
...
void CFileDriver_AVI::Submit()
{
	...
	m_DriverMgr_AVI.OpenFile(m_str);
	m_DriverMgr_AVI.StartInfoCheckThread();
	...
	m_DriverMgr_AVI.StartDecodeThread();
	m_DriverMgr_AVI.Run();
}

void CFileDriver_AVI::SetServiceName(CString strServiceNameOp)
{
	m_strServiceNameOp = strServiceNameOp;
}

UINT CFileDriver_AVI::TskCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_AVI* pParent = (CFileDriver_AVI*)pParam;
	pParent->m_startTime = timeGetTime();

//	pParent->ReportLog(TSK_IS_RUNING); // wang.zh add.
...
//	            glReportLog("Log_Task",strLog);
	...
	if (pParent->m_uReturnType == DR_USER_CANCEL)
	{
		...
		//pParent->ReportLog(TSK_IS_CANNELED);	// wang.zh add
	...
	if (pParent->m_uReturnType == DR_ERROR_ABORT)
	{
		...
		pParent->SetTaskStatus(TSK_IS_ABORTED);
		//pParent->ReportLog(TSK_IS_ABORTED);	//wang.zh add
