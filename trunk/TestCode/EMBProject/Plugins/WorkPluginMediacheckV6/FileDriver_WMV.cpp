// FileDriver_WMV.cpp: implementation of the CFileDriver_WMV class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
//#include "FCVSServer.h"
#include "FileDriver_WMV.h"
...
//////////////////////////////////////////////////////////////////////
REGISTER_CLASS(CFileDriver_WMV)
CFileDriver_WMV::CFileDriver_WMV() : CFileDriver(&m_DriverMgr_WMV)
{
	OutputDebugString("into CFileDriver_WMV::CFileDriver_WMV()\n");
	m_strTskID = "";
	m_strCheckKind = "";
	m_strFileType = "";
	m_strClipID = "";
...
//	m_DriverMgr_WMV.SetCallBack((JS_CCallBack*)this);
//	m_DriverMgr_WMV.SetBuffer((void*)&m_SampleList);
//	m_DriverMgr_WMV.SetParent(m_hWnd);
//	OutputDebugString("Leave CFileDriver_WMV::CFileDriver_WMV()\n");
}

CFileDriver_WMV::~CFileDriver_WMV()
{
	OutputDebugString("WWW:into CFileDriver_WMV::~CFileDriver_WMV()\n");
}

void CFileDriver_WMV::ShowMsg()
{
	AfxMessageBox(m_strMsg);
}
bool CFileDriver_WMV::InitDetectParam()
{
	...
	{
		CString strW,strH,strErr;
		strW.Format("%d",m_StreamInfo.VideoInfo.iVideowidth);
		strH.Format("%d",m_StreamInfo.VideoInfo.iVideoHeight);
       	XmlClipTypePara.LoadXMLFromString(strClipTypePara);
		XmlClipTypePara.SetAttribute("CLIP_WIDTH",strW,strErr,"Data");
		XmlClipTypePara.SetAttribute("CLIP_HEIGHT",strH,strErr,"Data");
	...
	if (GetFCVSServerConfig().m_bCheckAudio)
	{
		...
		m_pFileDriver->GetAudioMediaType(&mt);
		if (mt.FormatLength() >= sizeof(WAVEFORMATEX))//设置检测驱动的音频设置
		{
			memcpy(&WaveFormat,mt.Format(),mt.FormatLength());
			m_pAutoDetect->SetAudioType(WaveFormat.nChannels,WaveFormat.nSamplesPerSec,
...
BOOL CFileDriver_WMV::InitTask(CString strXML)
{
	OutputDebugString("into CFileDriver_WMV::InitTask(CString strXML)\n");
	if (strXML.IsEmpty())
	{
		return FALSE;
	}
...
// 	XmlOpReader.GetAttribute("ClipType",m_strClipType,strErr,"TaskData");
// 	CString strClipTypePara = (LPCSTR)GetFCVSServerConfig().GetDetectParaXml(m_strClipType);
//     CString strDriverPara   = GetFCVSServerConfig().GetXmlByClassName("CFileDriver_WMV");
...
// 	m_DriverMgr_WMV.SetBuffer((void*)&m_SampleList);
// 	if(GetFCVSServerConfig().m_bCheckAudio)
// 	   m_DriverMgr_WMV.SetAudioBuffer((void*)&m_AudioSampleList);
// 	m_DriverMgr_WMV.SetParent(m_hWnd);
// 
// 	m_SampleList.ReSet();
...
// 	///add end
	...
	OutputDebugString("leave CFileDriver_WMV::InitTask(CString strXML)\n");
	return TRUE;
}

BOOL CFileDriver_WMV::RunTask()
{
	BOOL bRet = m_DriverMgr_WMV.Run();
	if (bRet)
	{
...
BOOL CFileDriver_WMV::PauseTask()
{
	BOOL bRet = m_DriverMgr_WMV.Pause();
	if (bRet)
	{
...
BOOL CFileDriver_WMV::CancelTask()
{
	return m_DriverMgr_WMV.Cancel();
}

CString CFileDriver_WMV::GetTskID()
{
	return m_strTskID;
}

void CFileDriver_WMV::SetParentWnd(HWND hwnd)
{
	m_hParentWnd = hwnd;
}

LONGLONG CFileDriver_WMV::GetFileDuration()
{
	return m_StreamInfo.llFileDuration;
}

HRESULT CFileDriver_WMV::GetFileInfo(JS_StreamInfo p)
{
 	m_StreamInfo = p;
...
BOOL CFileDriver_WMV::GetFileInfoByArr(CStringArray& strFileInfoFiledArr,CStringArray& strFileInfoValueArr,BOOL bXML /* = TRUE */)
{
	CString strTemp;
	if (bXML)
	{
...
void CFileDriver_WMV::SetServiceName(CString strServiceNameOp)
{
	m_strServiceNameOp = strServiceNameOp;
}

void CFileDriver_WMV::Submit()
{
	...
	m_DriverMgr_WMV.OpenFile(m_str);
	m_DriverMgr_WMV.StartInfoCheckThread();
...
//	m_pAutoDetect->SetAudioType(m_StreamInfo.AudioInfo.lChannels,m_StreamInfo.AudioInfo.iAudioSamplingRate,m_StreamInfo.AudioInfo.lAudioQuantizationBits,nBlockAlign);
	m_DriverMgr_WMV.StartDecodeThread();

	if (!InitDetectParam())
	{
		SetTaskStatus(TSK_IS_ABORTED);
		CString strLog = "";
		strLog.Format("strTaskID = %s,ConText = %s(%s)",m_strTskID,"任务状态","初始化检测参数失败");
...
//	OutputDebugString("leave CFileDriver_WMV::Submit()");
}

UINT CFileDriver_WMV::TskCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_WMV* pParent = (CFileDriver_WMV*)pParam;
	...
	while (TRUE)
	{
		if (WaitForSingleObject(pParent->m_ThreadEvent,INFINITE) == WAIT_OBJECT_0)
		{
			...
			if (pMediaSample)
			{
				BYTE* p = pMediaSample->GetData();
				long  l = pMediaSample->GetLen();
				pParent->m_pAutoDetect->Detect(p , l);
			...
			else
			{
				...
				//增加超时判断
				nOutTime++;
				if(nOutTime > NUM_OUT_TIME)
				{
					pParent->m_uReturnType = DR_ERROR_ABORT;
					OutputDebugString("技审内容检测超时...");
					break;
				}
			}
		}
...
UINT CFileDriver_WMV::TskAudioCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_WMV* pParent = (CFileDriver_WMV*)pParam;
