// FileDriver_MSS.cpp: implementation of the CFileDriver_MSS class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
//#include "FCVSServer.h"
#include "FileDriver_MSS.h"
...
//////////////////////////////////////////////////////////////////////
int CFileDriver_MSS::nNum = 0;
REGISTER_CLASS(CFileDriver_MSS)
CFileDriver_MSS::CFileDriver_MSS() : CFileDriver(&m_DriverMgr_MSS)
{
	m_strTskID = "";
	m_strCheckKind = "";
	m_strFileType = "";
...
//	m_DriverMgr_MSS.SetCallBack((JS_CCallBack*)this);
//	m_DriverMgr_MSS.SetBuffer((void*)&m_SampleList);
//	m_DriverMgr_MSS.SetParent(m_hWnd);
}

CFileDriver_MSS::~CFileDriver_MSS()
{
	OutputDebugString("into CFileDriver_MSS::~CFileDriver_MSS()\n");
}

void CFileDriver_MSS::ShowMsg()
{
	AfxMessageBox(m_strMsg);
}

BOOL CFileDriver_MSS::InitTask(CString strXML)
{
	if (strXML.IsEmpty())
	{
		return FALSE;
    ...
    CString strDriverPara   = GetFCVSServerConfig().GetXmlByClassName("CFileDriver_MSS");
	...
	if(GetFCVSServerConfig().m_bCheckAudio)
	   m_DriverMgr_MSS.SetAudioBuffer((void*)&m_AudioSampleList);
	m_DriverMgr_MSS.SetParent(m_hWnd);
	///add end
...
BOOL CFileDriver_MSS::RunTask()
{
	BOOL bRet = m_DriverMgr_MSS.Run();
	if (bRet)
	{
...
BOOL CFileDriver_MSS::PauseTask()
{
	BOOL bRet = m_DriverMgr_MSS.Pause();
	if (bRet)
	{
...
BOOL CFileDriver_MSS::CancelTask()
{
	return m_DriverMgr_MSS.Cancel();
}

CString CFileDriver_MSS::GetTskID()
{
	return m_strTskID;
}

void CFileDriver_MSS::SetParentWnd(HWND hwnd)
{
	m_hParentWnd = hwnd;
}

HRESULT CFileDriver_MSS::GetFileInfo(JS_StreamInfo p)
{
	m_StreamInfo = p;
...
LONGLONG CFileDriver_MSS::GetFileDuration()
{
	return m_StreamInfo.llFileDuration;
}

BOOL CFileDriver_MSS::GetFileInfoByArr(CStringArray& strFileInfoFiledArr,CStringArray& strFileInfoValueArr,BOOL bXML /* = TRUE */)
{
	CString strTemp;
	if (bXML)
	{
...
void CFileDriver_MSS::Submit()
{
	...
	m_DriverMgr_MSS.OpenFile(m_str);
	m_DriverMgr_MSS.StartInfoCheckThread();
	...
	while(WaitForSingleObject(m_evFileInfoComplete,50) == WAIT_TIMEOUT)
	{
		...
		{
			CString strLog = "";
			strLog.Format("strTaskID = %s,ConText = %s(%s)",m_strTskID,"任务状态","文件信息检测失败");
//	        glReportLog("Log_Task",strLog);
	...
	m_DriverMgr_MSS.StartDecodeThread();
	m_DriverMgr_MSS.Run();
}

void CFileDriver_MSS::SetServiceName(CString strServiceNameOp)
{
	m_strServiceNameOp = strServiceNameOp;
}

UINT CFileDriver_MSS::TskCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_MSS* pParent = (CFileDriver_MSS*)pParam;
...
//	            glReportLog("Log_Task",strLog);
	...
	CString strLog123;
	strLog123.Format("\nmyOut123: Thread Out nReturnType = %d\n\n",pParent->m_uReturnType);
	OutputDebugString(strLog123);
	return 0;
}


UINT CFileDriver_MSS::TskAudioCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_MSS* pParent = (CFileDriver_MSS*)pParam;
