// FileDriver_MPG.cpp: implementation of the CFileDriver_MPG class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
//#include "FCVSServer.h"
#include "FileDriver_MPG.h"
...
REGISTER_CLASS(CFileDriver_MPG)
CFileDriver_MPG::CFileDriver_MPG() : CFileDriver(&m_DriverMgr_MPG)
{
	OutputDebugString("into CFileDriver_MPG::CFileDriver_MPG()\n");
	m_strTskID = "";
	m_strCheckKind = "";
	m_strFileType = "";
	m_strClipID = "";
...
//	m_DriverMgr_MPG.SetCallBack((JS_CCallBack*)this);
//	m_DriverMgr_MPG.SetBuffer((void*)&m_SampleList);
//	m_DriverMgr_MPG.SetParent(m_hWnd);
//	OutputDebugString("Leave CFileDriver_MPG::CFileDriver_MPG()\n");
}

CFileDriver_MPG::~CFileDriver_MPG()
{
	OutputDebugString("into CFileDriver_MPG::~CFileDriver_MPG()\n");
}

void CFileDriver_MPG::ShowMsg()
{
	AfxMessageBox(m_strMsg);
}

BOOL CFileDriver_MPG::InitTask(CString strXML)
{
	OutputDebugString("into CFileDriver_MPG::InitTask(CString strXML)\n");
	if (strXML.IsEmpty())
	{
		return FALSE;
	}
    ...
    CString strDriverPara   = GetFCVSServerConfig().GetXmlByClassName("CFileDriver_MPG");
	...
	m_SampleList.ReSet();
	nCount = 0;
	OutputDebugString("leave CFileDriver_MPG::InitTask(CString strXML)\n");
	return TRUE;
}

BOOL CFileDriver_MPG::RunTask()
{
	BOOL bRet = m_DriverMgr_MPG.Run();
	if (bRet)
	{
...
BOOL CFileDriver_MPG::PauseTask()
{
	BOOL bRet = m_DriverMgr_MPG.Pause();
	if (bRet)
	{
...
BOOL CFileDriver_MPG::CancelTask()
{
	return m_DriverMgr_MPG.Cancel();
}

CString CFileDriver_MPG::GetTskID()
{
	return m_strTskID;
}

void CFileDriver_MPG::SetParentWnd(HWND hwnd)
{
	m_hParentWnd = hwnd;
}

LONGLONG CFileDriver_MPG::GetFileDuration()
{
	return m_StreamInfo.llFileDuration;
}

HRESULT CFileDriver_MPG::GetFileInfo(JS_StreamInfo p)
{
 	m_StreamInfo = p;
...
BOOL CFileDriver_MPG::GetFileInfoByArr(CStringArray& strFileInfoFiledArr,CStringArray& strFileInfoValueArr,BOOL bXML /* = TRUE */)
{
	CString strTemp;
	if (bXML)
	{
...
void CFileDriver_MPG::SetServiceName(CString strServiceNameOp)
{
	m_strServiceNameOp = strServiceNameOp;
}

void CFileDriver_MPG::Submit()
{
	...
	m_DriverMgr_MPG.OpenFile(m_str);
	m_DriverMgr_MPG.StartInfoCheckThread();
	...
	m_DriverMgr_MPG.StartDecodeThread();
	m_DriverMgr_MPG.Run();
}

UINT CFileDriver_MPG::TskCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_MPG* pParent = (CFileDriver_MPG*)pParam;
