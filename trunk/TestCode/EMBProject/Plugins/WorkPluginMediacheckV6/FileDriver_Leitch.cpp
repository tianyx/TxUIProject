// FileDriver_Letch.cpp: implementation of the CFileDriver_Letch class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
//#include "FCVSServer.h"
#include "FileDriver_Leitch.h"
...
//////////////////////////////////////////////////////////////////////
REGISTER_CLASS(CFileDriver_Leitch)
CFileDriver_Leitch::CFileDriver_Leitch() : CFileDriver(&m_DriverMgr_LXF)
{
	OutputDebugString("into CFileDriver_Leitch::CFileDriver_Leitch()\n");
	m_strTskID = "";
	m_strCheckKind = "";
	m_strFileType = "";
	m_strClipID = "";
...
//	m_DriverMgr_LXF.SetCallBack((JS_CCallBack*)this);
//	m_DriverMgr_LXF.SetBuffer((void*)&m_SampleList);
//	m_DriverMgr_LXF.SetParent(m_hWnd);
//	OutputDebugString("Leave CFileDriver_Leitch::CFileDriver_Leitch()\n");
}

CFileDriver_Leitch::~CFileDriver_Leitch()
{
	OutputDebugString("into CFileDriver_WMV::~CFileDriver_Leitch()\n");
}

void CFileDriver_Leitch::ShowMsg()
{
	AfxMessageBox(m_strMsg);
}

BOOL CFileDriver_Leitch::InitTask(CString strXML)
{
	OutputDebugString("into CFileDriver_Leitch::InitTask(CString strXML)\n");
	if (strXML.IsEmpty())
	{
		return FALSE;
	}
    ...
    CString strDriverPara   = GetFCVSServerConfig().GetXmlByClassName("CFileDriver_Leitch");
	...
	if(GetFCVSServerConfig().m_bCheckAudio)
	   m_DriverMgr_LXF.SetAudioBuffer((void*)&m_AudioSampleList);
	m_DriverMgr_LXF.SetParent(m_hWnd);
	///add end
	...
	m_SampleList.ReSet();
	nCount = 0;
	OutputDebugString("leave CFileDriver_Leitch::InitTask(CString strXML)\n");
	return TRUE;
}

BOOL CFileDriver_Leitch::RunTask()
{
	BOOL bRet = m_DriverMgr_LXF.Run();
	if (bRet)
	{
...
BOOL CFileDriver_Leitch::PauseTask()
{
	BOOL bRet = m_DriverMgr_LXF.Pause();
	if (bRet)
	{
...
BOOL CFileDriver_Leitch::CancelTask()
{
	return m_DriverMgr_LXF.Cancel();
}

CString CFileDriver_Leitch::GetTskID()
{
	return m_strTskID;
}

void CFileDriver_Leitch::SetParentWnd(HWND hwnd)
{
	m_hParentWnd = hwnd;
}

LONGLONG CFileDriver_Leitch::GetFileDuration()
{
	return m_StreamInfo.llFileDuration;
}

HRESULT CFileDriver_Leitch::GetFileInfo(JS_StreamInfo p)
{
 	m_StreamInfo = p;
...
BOOL CFileDriver_Leitch::GetFileInfoByArr(CStringArray& strFileInfoFiledArr,CStringArray& strFileInfoValueArr,BOOL bXML /* = TRUE */)
{
	CString strTemp;
	if (bXML)
	{
...
void CFileDriver_Leitch::SetServiceName(CString strServiceNameOp)
{
	m_strServiceNameOp = strServiceNameOp;
}

void CFileDriver_Leitch::Submit()
{
	...
	m_DriverMgr_LXF.OpenFile(m_str);
	m_DriverMgr_LXF.StartInfoCheckThread();
	...
	m_DriverMgr_LXF.StartDecodeThread();
	m_DriverMgr_LXF.Run();
}

UINT CFileDriver_Leitch::TskCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_Leitch* pParent = (CFileDriver_Leitch*)pParam;
	...
	while (TRUE)
	{
		if (WaitForSingleObject(pParent->m_ThreadEvent,INFINITE) == WAIT_OBJECT_0)
		{
			...
			//	Sleep(5);
				...
				CString strLog;
				strLog.Format("myOut123: ThreadID = %d,Detect is Number %d\n\r",k,pParent->nCount);
			...
			//	HANDLE k = GetCurrentThread();
			//	CString strLog;
			//	strLog.Format("myOut123: ThreadID = %d,Sleep(5)\n\r",k);
			//	OutputDebugString(strLog);
				Sleep(5);
			}
		}
...
//	            glReportLog("Log_Task",strLog);
	...
	{
		OutputDebugString("myOut123: Detect User Cancel");
		pParent->SetTaskStatus(TSK_IS_CANNELED);
		//return 0;
	}
	if (pParent->m_uReturnType == DR_ERROR_ABORT)
	{
		OutputDebugString("myOut123: Detect Error Abort");
		pParent->SetTaskStatus(TSK_IS_ABORTED);
		//return 0;
	}
	CString strLog123;
	strLog123.Format("myOut123: Thread Out nReturnType = %d",pParent->m_uReturnType);
	OutputDebugString(strLog123);

	SetEvent(pParent->m_evThreadExit);
	return 0;
}

UINT CFileDriver_Leitch::TskAudioCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_Leitch* pParent = (CFileDriver_Leitch*)pParam;
