// FileDriver_Hik.cpp: implementation of the CFileDriver_Hik class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
//#include "FCVSServer.h"
#include "FileDriver_Hik.h"
...
//////////////////////////////////////////////////////////////////////
REGISTER_CLASS(CFileDriver_Hik)
CFileDriver_Hik::CFileDriver_Hik() : CFileDriver(&m_DriverMgr_Hik)
{
	OutputDebugString("into CFileDriver_Hik::CFileDriver_Hik()\n");
	m_strTskID = "";
	m_strCheckKind = "";
	m_strFileType = "";
	m_strClipID = "";
...
CFileDriver_Hik::~CFileDriver_Hik()
{
	OutputDebugString("into CFileDriver_WMV::~CFileDriver_Hik()\n");
}

void CFileDriver_Hik::ShowMsg()
{
	AfxMessageBox("ShowMsg()");
}

BOOL CFileDriver_Hik::InitTask(CString strXML)
{
	OutputDebugString("into CFileDriver_Hik::InitTask(CString strXML)\n");
	if (strXML.IsEmpty())
	{
		return FALSE;
	}
    ...
    CString strDriverPara   = GetFCVSServerConfig().GetXmlByClassName("CFileDriver_Hik");
	...
	m_DriverMgr_Hik.SetCallBack((JS_CCallBack*)this);
	m_DriverMgr_Hik.SetBuffer((void*)&m_SampleList);
	if(GetFCVSServerConfig().m_bCheckAudio)
	   m_DriverMgr_Hik.SetAudioBuffer((void*)&m_AudioSampleList);
	m_DriverMgr_Hik.SetParent(m_hWnd);
	///add end
	...
	OutputDebugString("leave CFileDriver_Hik::InitTask(CString strXML)\n");
	return TRUE;
}

BOOL CFileDriver_Hik::RunTask()
{
	BOOL bRet = m_DriverMgr_Hik.Run();
	if (bRet)
	{
...
BOOL CFileDriver_Hik::PauseTask()
{
	BOOL bRet = m_DriverMgr_Hik.Pause();
	if (bRet)
	{
...
BOOL CFileDriver_Hik::CancelTask()
{
	return m_DriverMgr_Hik.Cancel();
}

CString CFileDriver_Hik::GetTskID()
{
	return m_strTskID;
}

void CFileDriver_Hik::SetParentWnd(HWND hwnd)
{
	m_hParentWnd = hwnd;
}

LONGLONG CFileDriver_Hik::GetFileDuration()
{
	return m_StreamInfo.llFileDuration;
}

HRESULT CFileDriver_Hik::GetFileInfo(JS_StreamInfo p)
{
 	m_StreamInfo = p;
...
BOOL CFileDriver_Hik::GetFileInfoByArr(CStringArray& strFileInfoFiledArr,CStringArray& strFileInfoValueArr,BOOL bXML /* = TRUE */)
{
	CString strTemp;
	if (bXML)
	{
...
void CFileDriver_Hik::SetServiceName(CString strServiceNameOp)
{
	m_strServiceNameOp = strServiceNameOp;
}

void CFileDriver_Hik::Submit()
{
	...
	m_DriverMgr_Hik.OpenFile(m_str);
	m_DriverMgr_Hik.StartInfoCheckThread();
	...
	m_DriverMgr_Hik.StartDecodeThread();
	m_DriverMgr_Hik.Run();
}

UINT CFileDriver_Hik::TskCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_Hik* pParent = (CFileDriver_Hik*)pParam;
...
UINT CFileDriver_Hik::TskAudioCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_Hik* pParent = (CFileDriver_Hik*)pParam;
