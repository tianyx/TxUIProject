// FileDriver_TS.cpp: implementation of the CFileDriver_TS class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
//#include "FCVSServer.h"
#include "FileDriver_TS.h"
...
//////////////////////////////////////////////////////////////////////
REGISTER_CLASS(CFileDriver_TS)
CFileDriver_TS::CFileDriver_TS() : CFileDriver(&m_DriverMgr_TS)
{
	m_strTskID = "";
	m_strCheckKind = "";
	m_strFileType = "";
...
//	m_DriverMgr_TS.SetCallBack((JS_CCallBack*)this);
//	m_DriverMgr_TS.SetBuffer((void*)&m_SampleList);
//	m_DriverMgr_TS.SetParent(m_hWnd);
}

CFileDriver_TS::~CFileDriver_TS()
{
	OutputDebugString("into CFileDriver_TS::~CFileDriver_TS()\n");
}


void CFileDriver_TS::ShowMsg()
{
	AfxMessageBox(m_strMsg);
}
bool CFileDriver_TS::InitDetectParam()
{
...
BOOL CFileDriver_TS::InitTask(CString strXML)
{
	if (strXML.IsEmpty())
	{
		return FALSE;
...
// 	XmlOpReader.GetAttribute("ClipType",m_strClipType,strErr,"TaskData");
// 	CString strClipTypePara = (LPCSTR)GetFCVSServerConfig().GetDetectParaXml(m_strClipType);
//     CString strDriverPara   = GetFCVSServerConfig().GetXmlByClassName("CFileDriver_TS");
// 	CXMLOp XmlClipTypePara;
// 	CXMLOp XmlDrivePara;
...
// 	m_DriverMgr_TS.SetBuffer((void*)&m_SampleList);
// 	if(GetFCVSServerConfig().m_bCheckAudio)
// 	   m_DriverMgr_TS.SetAudioBuffer((void*)&m_AudioSampleList);
// 	m_DriverMgr_TS.SetParent(m_hWnd);
// 	m_SampleList.ReSet();
...
BOOL CFileDriver_TS::RunTask()
{
	BOOL bRet = m_DriverMgr_TS.Run();
	if (bRet)
	{
...
BOOL CFileDriver_TS::PauseTask()
{
	BOOL bRet = m_DriverMgr_TS.Pause();
	if (bRet)
	{
...
BOOL CFileDriver_TS::CancelTask()
{
	return m_DriverMgr_TS.Cancel();
}

CString CFileDriver_TS::GetTskID()
{
	return m_strTskID;
}

void CFileDriver_TS::SetParentWnd(HWND hwnd)
{
	m_hParentWnd = hwnd;
}

HRESULT CFileDriver_TS::GetFileInfo(JS_StreamInfo p)
{
	m_StreamInfo = p;
...
LONGLONG CFileDriver_TS::GetFileDuration()
{
	return m_StreamInfo.llFileDuration;
}

BOOL CFileDriver_TS::GetFileInfoByArr(CStringArray& strFileInfoFiledArr,CStringArray& strFileInfoValueArr,BOOL bXML /* = TRUE */)
{
	CString strTemp;
	if (bXML)
	{
...
void CFileDriver_TS::Submit()
{
	...
	m_DriverMgr_TS.OpenFile(m_str);
	m_DriverMgr_TS.StartInfoCheckThread();
...
//	m_pAutoDetect->SetAudioType(2,48000,16,4);
	m_DriverMgr_TS.StartDecodeThread();
...
void CFileDriver_TS::SetServiceName(CString strServiceNameOp)
{
	m_strServiceNameOp = strServiceNameOp;
}

UINT CFileDriver_TS::TskCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_TS* pParent = (CFileDriver_TS*)pParam;
...
UINT CFileDriver_TS::TskAudioCheckThread(LPVOID pParam)
{
	MyMediaSample* pMediaSample = NULL;
	CFileDriver_TS* pParent = (CFileDriver_TS*)pParam;
