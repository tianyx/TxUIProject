...
std::string mxflib::LookupDictionaryPath(const char *Filename)
{
	if(DictionaryPath == "~")
	{
		...
		if( !env ) 
			DictionaryPath = std::string( DEFAULT_DICT_PATH );
		else 
			DictionaryPath = std::string(env);
	}
...
std::string mxflib::SearchPath(const char *Path, const char *Filename)
{
	...
	if(*Filename == '.')
	{
		if(Filename[1] == DIR_SEPARATOR) 
			NonPath = true;
		else if((Filename[1] == '.') && (Filename[2] == DIR_SEPARATOR)) 
			NonPath = true;
	}
	...
	while(p && *p)
	{
		char *sep = strchr((char *)p, PATH_SEPARATOR);
