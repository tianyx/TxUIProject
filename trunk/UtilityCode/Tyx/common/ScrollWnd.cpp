...
#define IDC_SCROLLBAR_SKIN_H 9810
#define IDC_SCROLLBAR_SKIN_V 9811

BOOL g_bGotScrollLines = FALSE; // defined in wincore.cpp
...
public:
	...
	BOOL Create(CScrollWnd* pParent);
...
BOOL _AFX_MOUSEANCHORWND::Create(CScrollWnd* pParent)
{
	ENSURE_VALID(pParent);
	ASSERT_KINDOF(CScrollWnd, pParent);
...
BEGIN_MESSAGE_MAP(CScrollWnd, CWnd)
	...
	ON_WM_MOUSEWHEEL()
	//}}AFX_MSG_MAP
	ON_MESSAGE(WM_MBUTTONDOWN, &CScrollWnd::HandleMButtonDown)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_CREATE()
	ON_WM_KEYUP()
...
const AFX_DATADEF SIZE CScrollWnd::sizeDefault = {0,0};
...
void CScrollWnd::GetDeviceScrollSizes(int& nMapMode, SIZE& sizeTotal,
...
CSize CScrollWnd::GetWheelScrollDistance(CSize sizeDistance, BOOL bHorz, BOOL bVert)
{
	CSize sizeRet;
...
LRESULT CScrollWnd::HandleMButtonDown(WPARAM wParam, LPARAM lParam)
{
...
void CScrollWnd::UpdateBars()
{
	...
	// now update the bars as appropriate
	//modified tian_yx only vert scroll for im;
	if (!m_bHCrollbarEnabled)
	{
		needSb.cx = 0;
	}
...
void CScrollWnd::CheckScrollBars(BOOL& bHasHorzBar, BOOL& bHasVertBar) const
{
	DWORD dwStyle = GetStyle();
...
void CScrollWnd::PreSubclassWindow()
{
...
void CScrollWnd::EnableHScrollBar( BOOL bEnable /*= TRUE*/ )
{
	m_bHCrollbarEnabled = bEnable;
}



/*
void CScrollWnd::InitBarsSkins()
{
// 	if (!m_pVBarSkin)
// 	{
// 		m_pVBarSkin = new CSkinVerticleScrollbar;
// 		m_pVBarSkin->Create(NULL, WS_CHILD|WS_VISIBLE, CRect(0,0,10,10), this, IDC_SCROLLBAR_SKIN_V);
// 		m_pVBarSkin->pList = this;
// 	}
// 	if (!m_pHBarSkin)
// 	{
// 		m_pHBarSkin = new CSkinHorizontalScrollbar;
// 		m_pHBarSkin->Create(NULL, WS_CHILD|WS_VISIBLE, CRect(0,0,10,10), this, IDC_SCROLLBAR_SKIN_H);
// 		m_pHBarSkin->pList = this;
// 	}


}

void CScrollWnd::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	UpdateSkinBar();
...
void CScrollWnd::PositionScrollBars()
{
	...
	if (!m_pHBarSkin || !m_pHBarSkin->GetSafeHwnd())
	{
		return;
	}
	...
	CRect rcHbar(windowRect);
	rcHbar.top = rcHbar.bottom - m_cyHScroll;
	rcHbar.right -= m_cxVScroll;
	m_pHBarSkin->SetWindowPos(NULL, rcHbar.left, rcHbar.top, rcHbar.Width(), rcHbar.Height(), SWP_NOACTIVATE);
	CRect rcVbar(windowRect);
	rcVbar.left = rcVbar.right- m_cxVScroll;
	m_pVBarSkin->SetWindowPos(NULL, rcVbar.left, rcVbar.top, rcVbar.Width(), rcVbar.Height(), SWP_NOACTIVATE);


	m_pHBarSkin->UpdateThumbPosition();
	m_pVBarSkin->UpdateThumbPosition();
}

void CScrollWnd::UpdateSkinBar()
{
	if (!m_pVBarSkin)
	{
		return;
