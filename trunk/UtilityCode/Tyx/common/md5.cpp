...
static void MD5Transform (UINT4 par1[4], unsigned char par2[64]);
static void Encode (unsigned char *par1, UINT4 * par2, unsigned int par3);
static void Decode (UINT4 * par1, unsigned char * par2, unsigned int par3);
static void MD5_memcpy  (POINTER par1, POINTER par2, unsigned int par3);
static void MD5_memset  (POINTER par1, int par2, unsigned int par3);
...
wchar_t* WMDString (wchar_t *string)
{	
	int nLen = wcslen(string);
	char* strTmp = new char[nLen + 1];
	sprintf_s(strTmp, nLen +1 , "%S", string);
	char* szRet = MDString(strTmp);
	delete[] strTmp;
	static wchar_t woutput[35]={L""};
	swprintf_s(woutput, 35, L"%s", szRet);
	return woutput;
}
...
{ static char output[33]={""};
  ...
  if ((file = fopen (filename, "rb")) == NULL)
   { printf ("%s can't be openedn", filename);
    ...
    fclose (file);
    for (i = 0; i < 16; i++)
     {sprintf(&(output1[2*i]),"%02x",(unsigned char)digest[i] & 0x0F);
        sprintf(&(output1[2*i+1]),"%02x",(unsigned char)(digest[i]<<4));
          }
        for(i=0;i<32;i++)
       output[i]=output1[i];
